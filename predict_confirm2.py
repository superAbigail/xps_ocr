...
def cut_img(xps_img, tp, bt, lf, rt):
    img_shape = xps_img.shape
    wide1 = int(img_shape[1] * lf)
    wide2 = int(img_shape[1] * rt)
    long1 = int(img_shape[0] * tp)
    long2 = int(img_shape[0] * bt)
    # return xps_img[751: 1007, 184: 823]
    xps_img = xps_img[long1: long2, wide1: wide2]
    ...
    for image_file in image_file_list:
        ...
        for i, re in enumerate(rec_res):
            if '确认报告' in re[0]:
                ...
                break
        ratio_rc = 0.023375
        ratio_judge1_top = ratio_cf / ori_img.shape[0] + ratio_rc
        ratio_judge1_bot = ratio_judge1_top + 0.0131129
        ratio_judge1_lefe = 0.292
        ratio_judge1_right = 0.3339

        judge1 = cut_img(ori_img, ratio_judge1_top, ratio_judge1_bot, ratio_judge1_lefe, ratio_judge1_right)
        j1, _ = te([judge1])

        ratio_judge2_top = ratio_judge1_top
        ratio_judge2_bot = ratio_judge1_bot
        ratio_judge2_lefe = 0.4758
        ratio_judge2_right = 0.6572581

        judge2 = cut_img(ori_img, ratio_judge2_top, ratio_judge2_bot, ratio_judge2_lefe, ratio_judge2_right)
        j2, _ = te([judge2])
